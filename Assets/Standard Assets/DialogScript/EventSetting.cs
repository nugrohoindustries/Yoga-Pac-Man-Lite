...
//	public AnimationName animName;
	...
	public MonoBehaviour targetScript;   // Drag the script component you want to activate
    public string methodName = "Marah";
	...
	public void Activate(){
		...
		if(type == ParametersType.PlayMecanimAnimation){
			...
			//	t.GetComponent<Animator>().Play(animClip.name);
				t.GetComponent<Animator>().Play(animName);
			}
		...
		//	Application.LoadLevel(stringVar);
			SceneManager.LoadScene(stringVar);
			NextEvent();
		}
	...
	void StartDialogue(){
		...
		if(!target){
			target = GetComponent<EventActivator>().audios.transform;
		...
		if(!showText){
			showText = GetComponent<EventActivator>().DialogText;
		}
		...
		if(!textBoxVar){
			textBoxVar = GetComponent<EventActivator>().textBoxVar;
		}
		if(!nameText){
			nameText = GetComponent<EventActivator>().nameText;
		}
		if(!autoSkip){
			autoSkip = GetComponent<EventActivator>().autoSkip;
		}
		if(!ff){
			ff = GetComponent<EventActivator>().ff;
		}
	...
	//	autoSkip.GetComponent<AnimateText>().autoSkip = false;
	}
	
//	var audi : target.GetComponent<AudioSource>().PlayOneShot(audiClip);
